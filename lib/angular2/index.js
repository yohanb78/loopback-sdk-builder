...
module.exports = function generate(ctx) {
  ...
  **/
      ...
      if (postData.length > 0) {
        ...
        postData.forEach((property, i) => {
          if (property.http.source == 'form') {
            ...
            output.push(`      ${property.arg}${!property.required ? '?' : ''}: ${property.arg}${(i < l - 1) ? ',' : ''}`);
          }
        });
   ...
   * @description
   * Define which properties are not required, if avlue undefined or null don't add to body while posting data (POST, PUT, PATCH)
   */
  function postBodyDeleteNotRequiredProperty(postData) {
    let output = [];
    if (Array.isArray(postData)) {
      postData = postData.filter(param => {
        // Filter out route params and function params
      ...
      if (postData.length > 0) {
        ...
        postData.forEach((property, i) => {
          if (property.http.source === 'body' && !property.required) {
            output.push(`      if (${property.arg} === null || ${property.arg} === undefined) delete _postBody.${property.arg}`);
          }
        });
        output.push('');
      }
