      ...
      if (postData.length > 0) {
        ...
        postData.forEach((property, i) => {
          if (property.http.source == 'form') {
            ...
            output.push(`      ${property.arg}${(property.required ?? 'true') !== 'true' ? '?' : ''}: ${property.arg}${(i < l - 1) ? ',' : ''}`);
          }
        });
        if (formData.length > 0){
          ...
          postData.forEach((property, i) => {
            if (property.http.source === 'body' && (property.required ?? 'true') !== 'true') {
              output.push(`if (${property.arg} === undefined || ${property.arg} === null) delete _postBody.${property.arg};`);
            }
          });
        }
        output.push('    ');
