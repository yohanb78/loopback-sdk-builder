  ...
  **/
      ...
      if (ctx.fireloopOnly === 'disabled' || (ctx.models[modelName].sharedClass.ctor.settings.base === 'User')) {
        schema.push({
          ...
          params: {
            ...
            buildPostBody,
            postBodyDeleteNotRequiredProperty,
            buildUrlParams,
            buildRouteParams,
            buildMethodParams,
            buildPayloadParams,
  ...
  function buildModelProperties(model, isInterface) {
    ...
    Object.keys(model.properties).forEach((propertyName) => {
      ...
      output.push(`  "${camelCase(propertyName)}"${isOptional}: ${buildPropertyType(property)}${defaultValue};`);
    });
    // Add Model Relations
  ...
  function buildMethodParams(model, methodName, params, isIo) {
    ...
    params.forEach((param, i, arr) => {
      ...
      if (!param.required && (model.isUser && methodName === 'login') && param.arg === 'include') {
        type = 'any';
        value = " = 'user'";
      } else if (type.match(/LoopBackFilter/)) {
        value = !param.required ? ` = ${ isArray ? '[]' : '{}' }`: '';
      } else if (type.match(/any/)) {
        value = !param.required ? ` = ${ isArray ? '[]' : 'undefined' }`: '';
      } else {
        value = !param.required ? ` = ${ isArray ? `new Array<${type}>()` : `new ${type}()` }`: '';
      }
      output.push(`${camelCase(param.arg)}: ${type}${ isArray ? '[]' : '' }${value}`);
    });
      ...
      if (postData.length > 0) {
        ...
        if (formData.length > 0){
          ...
          formData.forEach((property, i) => {
            output.push(`        "${property.arg}": ${camelCase(property.arg)}${(i < l - 1) ? ',' : ''}`);
          })
          output.push(`      }`);
        }
        output.push('    ');
  ...
  /**
   * @method postBodyDeleteNotRequiredProperty
   * @description
   * Define which properties are not required, if value is undefined or null don't add to body while posting data (POST, PUT, PATCH)
   */
  function postBodyDeleteNotRequiredProperty(postData, propertyPostData) {
    let output = [];
    if (Array.isArray(postData)) {
      postData = postData.filter(param => {
        // Filter out route params and function params
      ...
      if (postData.length > 0) {
        output.push('');
        postData.forEach((property, i) => {
          if (['body', 'form'].includes(property.http.source) && !property.required) {
            output.push(`    if (${property.arg} === null || ${property.arg} === undefined) delete ${property.http.source === 'body' ? propertyPostData : 'data'}.${property.arg};`);
          }
        });
        output.push('');
      }
